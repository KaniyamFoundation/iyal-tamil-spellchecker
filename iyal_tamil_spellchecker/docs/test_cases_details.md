...
*   **`test_derived_whitelist_words`**: Rigorously tests plural, instrumental, genitive, locative, comitative, and adverbial modifiers (e.g., `அருட்கொடைகளால்`, `அருட்கொடைகளின்`, `சுரேஷின்`, `குர்ஆனாகவே`) mapping back appropriately.
    *   **Doubled-Consonant Cases**: Validates support for markers like `-ட்டை`, `-த்தை`, `-ப்பை`, `-க்கை`, `-ச்சை` (e.g., `ஈஸ்ட்வுட்டை`, `சோதனைரூப்பைக்`).
...
### 5. Regression & Edge Case Validation
**File:** `test_regression_cases.py`
Captures historical failures and specific user-reported bugs to prevent regression.
...
Validates the remote update notification system.
*   **`test_version_comparison_logic`**: Ensures the UI correctly displays the update banner when the local version is behind GitHub.
*   **`test_version_check_caching`**: Verifies that the remote version is only fetched once per hour to optimize performance.
*   **`test_version_check_failure_graceful`**: Confirms that network errors during the version check do not affect the main spellchecker functionality.
...
## 🖥️ Frontend Editor Capabilities
The interactive web client exposes dedicated browser controls.
*   **Undo State Manager**: Custom `undoStack` implementation providing an exact 2-tier backup boundary triggered natively via keyboard `Ctrl+Z` mappings.
